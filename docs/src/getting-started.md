...
`/usr/share/zoneinfo`. See [Compatibility](reference/compatibility.md) for how
that lookup works and what `TZDIR` overrides.
...
See [Core concepts](guide/core-concepts.md) for the full type layering and
[API reference](reference/api.md) for every function.
