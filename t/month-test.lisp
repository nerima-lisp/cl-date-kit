...
(describe
  ...
  (it
    ...
    (loop
      for value from 1 to 12
      for month = (cl-date-kit:month-from-value value)
      do (expect (cl-date-kit:month-value month) :to-be value)
         (expect (cl-date-kit:month-plus month 12) :to-be month)
         (expect (cl-date-kit:month-minus month 12) :to-be month)))
  (it
  ...
  (it
    ...
    (expect (cl-date-kit:month-from-local-date (make-local-date 2024 2 29))
            :to-be
            :february))
  (it
