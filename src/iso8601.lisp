...
(defun format-local-date-time (dt)
  (concatenate
    'string
    (format-local-date (local-date-time-date dt))
    "T"
    (format-local-time (local-date-time-time dt))))
...
(defun %parse-local-date-time-and-offset (string)
  ...
  (let ((t-pos (or (position #\T string) (position #\t string))))
    ...
    (let ((offset-start
          (position-if
            (lambda (character)
              (or (char= character #\+) (char= character #\-) (char= character #\Z) (char= character #\z)))
            string
            :start
            (1+ t-pos))))
      (unless offset-start
...
(defun format-instant (instant)
  (concatenate
    'string
    (format-local-date-time (local-date-time-of-instant instant (zone-offset-utc)))
    "Z"))

(progn
  (defun %canonical-utc-instant-p (string)
    (and
     (stringp string)
     (= (length string) 20)
     (char= (char string 4) #\-)
  ...
  (defun %parse-canonical-utc-instant (string)
    ...
    (with-date-time-parse-error
      ...
      (labels ((digit-at (index)
               (or (digit-char-p (char string index))
  ...
  (defun parse-instant (string)
    (if (%canonical-utc-instant-p string)
      ...
      (multiple-value-bind (sign absolute) (if (minusp total) (values "-" (- total))
        ...
        (multiple-value-bind (hours remainder) (floor absolute 3600)
          (multiple-value-bind (minutes seconds) (floor remainder 60)
            (if (zerop seconds) (format nil "~A~2,'0D:~2,'0D" sign hours minutes)
            ...
            (cond
              ...
              ((and (>= length 3) (member (char string start) '(#\+ #\-)))
               (let ((sign (if (char= (char string start) #\-) -1 1)))
                 (handler-case
                     (cond
                       ((and (= length 3) (digits-at-p '(1 2)))
                        (parse-components sign 1 3))
                       ((and (= length 5) (digits-at-p '(1 2 3 4)))
                        (parse-components sign 1 3 3 5))
                       ((and (= length 6)
                       ...
                       (t (invalid-input)))
                   (invalid-zone-offset () (invalid-input)))))
              (t (invalid-input))))
          (invalid-input)))))
...
(defun format-offset-date-time (offset-date-time)
  (concatenate
    'string
    (format-local-date-time (offset-date-time-local-date-time offset-date-time))
    (format-zone-offset (offset-date-time-offset offset-date-time))))
...
(defun format-offset-time (offset-time)
  (concatenate
    'string
    (format-local-time (offset-time-local-time offset-time))
    (format-zone-offset (offset-time-offset offset-time))))
...
When a zone suffix is present, its rules must admit the explicit offset."
  (unless (stringp string)
    (error 'date-time-parse-error
           :string string
           :expected "YYYY-MM-DDTHH:MM:SS+HH:MM[Zone/Id]"))
  (let* ((t-pos (or (position #\T string) (position #\t string)))
    ...
    (let* ((offset-end (or open-bracket length))
      ...
      (let* ((local (parse-local-date-time (subseq string 0 sign-pos)))
             ...
             (zone (if open-bracket
                       (handler-case
                           (find-time-zone
                            (subseq string (1+ open-bracket) close-bracket))
                         (cl-date-kit-error ()
                           (error 'date-time-parse-error
                                  :string string
                                  :expected "known Zone/Id")))
                       offset)))
        (when open-bracket
          (unless (%local-date-time-has-offset-p local zone offset)
            (error 'date-time-parse-error
                   :string string
                   :expected "offset valid for Zone/Id")))
