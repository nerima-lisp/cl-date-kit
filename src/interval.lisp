...
(defstruct (interval (:constructor %make-interval (start end))) (start nil :type instant :read-only t)
  (end nil :type instant :read-only t))
...
The result is a list containing zero, one, or two half-open intervals."
  (let ((shared (interval-intersection interval removed)))
    (if (and shared (not (interval-empty-p shared))) (remove
        nil
        (list
...
(defstruct (local-date-interval (:constructor %make-local-date-interval (start end))) (start nil :type local-date :read-only t)
...
(defun local-date-interval-empty-p (interval)
  ...
  (local-date=
    (local-date-interval-start interval)
...
exclusive end.  Returns NIL when exhausted or stopped."
  ...
  (loop for date = (local-date-interval-start interval) then (local-date-plus-days date 1)
...
(defun local-date-interval-contains-p (interval date)
  ...
  (and
    (local-date<= (local-date-interval-start interval) date)
...
(defun local-date-interval-encloses-p (outer inner)
  ...
  (and
    (local-date<=
      (local-date-interval-start outer)
      (local-date-interval-start inner))
    (local-date<= (local-date-interval-end inner) (local-date-interval-end outer))))

(defun local-date-interval-overlaps-p (a b)
  "Whether non-empty intervals A and B share at least one calendar date."
  (and
    (local-date< (local-date-interval-start a) (local-date-interval-end b))
...
(defun local-date-interval-connected-p (a b)
  "Whether A and B overlap or abut without a positive gap."
  (or (local-date-interval-overlaps-p a b) (local-date-interval-abuts-p a b)))
...
(defun local-date-interval-intersection (a b)
  ...
  (when (local-date-interval-overlaps-p a b)
    (make-local-date-interval
      (if (local-date> (local-date-interval-start a) (local-date-interval-start b)) (local-date-interval-start a)
        (local-date-interval-start b))
      (if (local-date< (local-date-interval-end a) (local-date-interval-end b)) (local-date-interval-end a)
...
(defun local-date-interval-difference (interval removed)
  ...
  (let ((shared (local-date-interval-intersection interval removed)))
    (if (and shared (not (local-date-interval-empty-p shared))) (remove
        nil
        (list
          (when (local-date<
              (local-date-interval-start interval)
            ...
            (make-local-date-interval
              ...
              (local-date-interval-start shared)))
          (when (local-date<
              (local-date-interval-end shared)
              (local-date-interval-end interval))
            (make-local-date-interval
              (local-date-interval-end shared)
              (local-date-interval-end interval)))))
