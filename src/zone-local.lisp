...
(defun %tzif-maximum-absolute-offset-seconds (data) (let ((maximum (abs (tzif-type-utc-offset (tzif-data-initial-type data))))) (loop for type across (tzif-data-transition-types data) do (setf maximum (max maximum (abs (tzif-type-utc-offset type))))) maximum))

(defun %time-zone-local-search-radius (zone) (or (time-zone-local-search-radius zone) (setf (time-zone-local-search-radius zone) (%tzif-maximum-absolute-offset-seconds (time-zone-tzif-data zone)))))

(defun %time-zone-local-transition-index-range (zone naive-seconds) (let* ((times (tzif-data-transition-times (time-zone-tzif-data zone))) (radius (%time-zone-local-search-radius zone)) (start (1+ (%binary-search-le times (1- (- naive-seconds radius)))))) (values start (%binary-search-le times (+ naive-seconds radius)))))
    ...
    (time-zone
     (let* ((data (time-zone-tzif-data zone))
       ...
       (multiple-value-bind (start end)
           (%time-zone-local-transition-index-range zone naive)
         (flet ((type-before (index)
                  (if (zerop index)
                      (tzif-data-initial-type data)
                      (aref types (1- index)))))
           (loop
             ...
             when kind
               do (return-from %classify-local-date-time
                    (values kind
       ...
       (let ((rule (time-zone-posix-rule zone)))
         (when (and rule (%posix-rule-applies-to-local-p zone naive))
           (multiple-value-bind (kind first-offset second-offset)
               (%classify-posix-local-date-time naive rule)
             (when kind
               (return-from %classify-local-date-time
                 (values kind first-offset second-offset))))))
       (let ((type (%time-zone-type-for-instant zone naive)))
         (values :normal
                 (if type
                     ...
                     (%make-zone-offset
                      (nth-value 0
                                 (%posix-state-values-at-instant
                                  naive
                                  (time-zone-posix-rule zone)))))
                 nil))))))
    ...
    (etypecase zone
      ...
      (time-zone
       (let* ((naive-seconds
                (%local-date-time->naive-seconds local-date-time)))
         (multiple-value-bind (start end)
             (%time-zone-local-transition-index-range zone naive-seconds)
           (or
            (loop
              for index from start to end
            ...
            (let ((rule (time-zone-posix-rule zone)))
              (when (and rule
                         (%posix-rule-applies-to-local-p zone naive-seconds))
                (let ((year
                        (local-date-year
                         (local-date-from-epoch-day
                          (floor naive-seconds 86400)))))
                  (loop
                    for transition-year from (1- year) to (1+ year)
                    do (dolist (description
                                (%posix-transitions-for-year
                         ...
                         (destructuring-bind (boundary before after) description
                           (let ((transition
                             ...
                             (when (and transition
                                        (%posix-transition-applies-p
                                         zone boundary)
                                        (applies-p transition naive-seconds))
                               (return-from local-date-time-zone-transition
                                 transition))))))))))))))))
...
(defun local-date-of-instant (instant zone)
  ...
  (local-date-time-date (local-date-time-of-instant instant zone)))

(defun local-time-of-instant (instant zone)
  "Applies the offset in force for ZONE at INSTANT and returns the local time."
  (local-date-time-time (local-date-time-of-instant instant zone)))
