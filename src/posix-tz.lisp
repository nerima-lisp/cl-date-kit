...
(defstruct posix-tz-rule
  (std-utc-offset 0 :type integer :read-only t)
  ...
  (transitions-cache (make-hash-table :test (function eql)) :type hash-table :read-only t))
...
(defun %parse-posix-name-offset (segment pos offset-required-p invalid)
  ...
  (let ((length (length segment)))
    ...
    (let ((name-end
      ...
      (let ((name (if (char= (char segment pos) #\<)
                      (subseq segment (1+ pos) (1- name-end))
...
(defun %posix-state-values-at-instant (epoch rule)
  (if (null (posix-tz-rule-dst-start rule))
      (values (posix-tz-rule-std-utc-offset rule)
              (posix-tz-rule-std-name rule)
              nil)
      (let* ((approx-year
               (local-date-year
        ...
        (loop for year from (- approx-year 2) to (1+ approx-year)
              do (dolist (transition (%posix-transitions-for-year year rule))
                   (when (and (<= (first transition) epoch)
                              (or (null latest-transition)
                                  (> (first transition)
                                     (first latest-transition))))
                     (setf latest-transition transition))))
        (if (= (third latest-transition) (posix-tz-rule-dst-utc-offset rule))
            (values (posix-tz-rule-dst-utc-offset rule)
                    (posix-tz-rule-dst-name rule)
                    t)
            (values (posix-tz-rule-std-utc-offset rule)
                    (posix-tz-rule-std-name rule)
                    nil)))))
...
(defun %posix-transitions-for-year (year rule)
  ...
  (let ((cache (posix-tz-rule-transitions-cache rule)))
    (multiple-value-bind (cached present-p) (gethash year cache)
      (if present-p
          cached
          (setf (gethash year cache)
                (if (and (posix-tz-rule-dst-start rule)
                         (posix-tz-rule-dst-end rule))
                    (let ((standard-offset (posix-tz-rule-std-utc-offset rule))
...
(defun %classify-posix-local-date-time (naive-seconds rule)
  (let ((year
          (local-date-year
           (local-date-from-epoch-day (floor naive-seconds 86400)))))
    (block found
      (loop
    ...
    (labels ((invalid (control &rest arguments)
      ...
      (let ((segments (%split-string string #\,)))
        ...
        (let ((zone-segment (first segments)))
          (multiple-value-bind (std-name std-offset std-pos)
...
(defun %parse-posix-rule (string invalid)
  ...
  (let* ((slash (position #\/ string))
         (date-part (if slash (subseq string 0 slash) string))
    ...
    (multiple-value-bind (second-of-day mode)
        (if time-part
            (%parse-posix-rule-time time-part invalid)
            (values 7200 :wall))
      (labels ((decimal (text)
                 ...
                 (parse-integer text)))
        (cond
          ((and (plusp (length date-part))
                (char= (char date-part 0) #\M))
