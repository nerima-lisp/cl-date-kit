...
(defun %write-pattern-year (stream year width)
  ...
  (if (and (>= width 4) (or (minusp year) (> year 9999)))
      (format stream "~A~v,'0D"
              (if (minusp year) "-" "+")
              (max 4 width)
              (abs year))
          ...
          (multiple-value-bind (hours remainder) (floor absolute 3600)
            (multiple-value-bind (minutes seconds) (floor remainder 60)
              ...
              (ecase width
                ...
                (3 (if (zerop seconds)
                       (format stream "~C~2,'0D:~2,'0D" sign hours minutes)
                       (format stream "~C~2,'0D:~2,'0D:~2,'0D"
                               sign hours minutes seconds))))))))))

(defun %write-pattern-field (stream field width date time offset zone instant pattern locale)
  (case field
    (#\y
     (%write-pattern-year stream
    ...
    (#\M
     (let ((month (local-date-month (%require-pattern-field date pattern field "a date"))))
       (if (< width 3)
           (%write-pattern-number stream month width)
           (write-string (%date-time-locale-month-name locale month width) stream))))
    (#\d
     (%write-pattern-number stream
    ...
    (#\S
     (%write-pattern-number
      stream
      (floor (local-time-nanosecond (%require-pattern-field time pattern field "a time"))
             (expt 10 (- 9 width)))
      width))
    (#\X
     (%write-pattern-offset stream
    ...
    (#\V
     (let ((resolved-zone (%require-pattern-field zone pattern field "an IANA time zone")))
       (if (time-zone-p resolved-zone)
           (write-string (time-zone-name resolved-zone) stream)
  ...
  (cond
    ...
    ((offset-date-time-p value)
     (values (offset-date-time-date value) (offset-date-time-time value)
             (offset-date-time-offset value) nil nil))
    ...
    ((instant-p value)
     (let* ((resolved-zone (or zone (zone-offset-utc)))
            (local (local-date-time-of-instant value resolved-zone))
            (offset (if (time-zone-p resolved-zone)
                        (offset-for-instant resolved-zone value)
                        resolved-zone)))
       (values (local-date-time-date local) (local-date-time-time local)
               offset resolved-zone value)))
