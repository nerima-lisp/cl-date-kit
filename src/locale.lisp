...
(progn
  (defparameter +english-date-time-locale+ (%make-date-time-locale
      ...
      "AM"
      "PM"))
  (defparameter +japanese-date-time-locale+ (%make-date-time-locale
      :ja
      #("1月"
        "2月"
        "3月"
...
(progn
  (defun %copy-date-time-locale-names (names expected-length)
    ...
    (unless (and (= (length names) expected-length) (every #'stringp names))
      (error
        'type-error
        :datum
        names
        :expected-type
        `(vector string ,expected-length)))
    (map 'vector #'copy-seq names))
  (defun %non-empty-string-p (value)
    (and (stringp value) (plusp (length value))))
  (defun %copy-date-time-locale-meridiem (value)
    (unless (%non-empty-string-p value)
      (error 'type-error :datum value :expected-type '(satisfies %non-empty-string-p)))
    (copy-seq value))
  (defun make-date-time-locale (&key name short-months months short-weekdays weekdays am pm)
...
LOCALE can be a custom DATE-TIME-LOCALE or the bundled :EN or :JA identifier.
PATTERN is an internal diagnostic context."
    (cond
      ((date-time-locale-p locale) locale)
      ((eq locale :en) +english-date-time-locale+)
      ((eq locale :ja) +japanese-date-time-locale+)
      (t
        (error
          ...
          pattern
          :reason
          (format nil "unsupported locale ~S; available locales are :EN and :JA" locale))))))
