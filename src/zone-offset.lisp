...
(defstruct (zone-transition
    (:constructor %make-zone-transition (instant offset-before offset-after))) (instant nil :type instant :read-only t)
...
(defun zone-offset-of-total-seconds (total-seconds)
  "Constructs a validated ISO-8601 UTC offset from TOTAL-SECONDS in the inclusive range -64800 to +64800."
  (unless (integerp total-seconds)
    (error (quote invalid-zone-offset) :hours total-seconds :minutes 0 :seconds 0))
  (let ((sign (signum total-seconds)))
    (multiple-value-bind (hours remainder) (floor (abs total-seconds) 3600)
      (multiple-value-bind (minutes seconds) (floor remainder 60)
        (zone-offset-of-hms (* sign hours) (* sign minutes) (* sign seconds))))))

(defun zone-offset-utc ()
  (%make-zone-offset 0))

(defun zone-offset-compare (a b)
  "-1, 0, or 1 as fixed offset A is less than, equal to, or greater than B."
  (let ((delta (- (zone-offset-total-seconds a) (zone-offset-total-seconds b))))
    (cond
      ((minusp delta) -1)
      ((plusp delta) 1)
      (t 0))))
